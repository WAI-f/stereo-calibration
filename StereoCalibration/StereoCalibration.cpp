...
static void StereoCalib(const char* imageList, int nx, int ny,
    ...
    bool displayCorners = false;    // display corner detection result or not     
    bool showUndistorted = true;    
    bool isVerticalStereo = false;  // cameras place type: left/right or top/bottom
    const int maxScale = 1;         // image scale factor
    const float squareSize = 1.f;   // chessboard square size

    // actual square size
    int i, j, lr;
    int N = nx * ny;                            // chessboard corner number
    cv::Size board_sz = cv::Size(nx, ny);       // chessboard corner size
    
    vector<string> imageNames[2];
    vector<cv::Point3f> boardModel;             // 3D points in chessboard
    vector<vector<cv::Point3f> > objectPoints;  // 3D chessboard corner for all pair images
    vector<cv::Point2f> corners[2];             // 2D points in chessboard
    vector<vector<cv::Point2f> > points[2];     // 2D chessboard corner for all pair images
    ...
    // set chessboard corners 3D coordinate
    for (i = 0; i < ny; i++)
        for (j = 0; j < nx; j++)
            boardModel.push_back(
                cv::Point3f((float)(i * squareSize), (float)(j * squareSize), 0.f));
    // read txt to get images path and load images
    i = 0;
    for (;;) {
        char buf[1024];
        lr = i % 2;
        ...
        for (int s = 1; s <= maxScale; s++) {
            ...
            if (found[lr] || s == maxScale) {
                cv::Mat mcorners(corners[lr]);  // shallow copy
                mcorners *= (1. / s);
            }
            if (found[lr])
                break;
    ...
    for (i = 0; i < nframes; i++) {
        vector<cv::Point2f>& pt0 = points[0][i];                // left image corner points
        vector<cv::Point2f>& pt1 = points[1][i];                // right image corner points
        cv::undistortPoints(pt0, pt0, M1, D1, cv::Mat(), M1);   // undistort left image corner points
        cv::undistortPoints(pt1, pt1, M2, D2, cv::Mat(), M2);   // undistort right image corner points
        cv::computeCorrespondEpilines(pt0, 1, F, lines[0]);     // calcu right epilines of corners in left image
        cv::computeCorrespondEpilines(pt1, 2, F, lines[1]);     // calcu left epilines of corners in right image (ax+by+c=0)

        // |ar*xr+br*yr+zr|+|al*xl+bl*yl+zl|
        for (j = 0; j < N; j++) {
